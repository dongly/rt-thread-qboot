...
static int nor_flash_memory_mapped_mode(struct rt_qspi_device *device)
{
    ...
    if (HAL_QSPI_MemoryMapped((QSPI_HandleTypeDef *)(device->parent.bus->parent.user_data),
                              &s_command, &s_mem_mapped_cfg)
        != HAL_OK)
    {
...
void qbt_jump_to_app(void)
{
    ...
    if (((reset_handler & 0xff000000) != QSPI_BASE)
        || (stk_addr != 0x20000000 && stk_addr != 0x24000000 && stk_addr != 0x30000000
            && stk_addr != 0x38000000))
    {
...
void qbt_jump_to_app(void)
{
    ...
    if ((((u32)app_func & 0xff000000) != 0x08000000)
        || (((stk_addr & 0x2ff00000) != 0x20000000) && ((stk_addr & 0x2ff00000) != 0x24000000)))
    {
        LOG_E("No legitimate application.");
        return;
    }
