...
#if (defined(QBOOT_USING_AES) || defined(QBOOT_USING_GZIP) || defined(QBOOT_USING_QUICKLZ)         \
     || defined(QBOOT_USING_FASTLZ))
...
static check_func_t factory_check_hook = NULL;
...
static bool qbt_fw_crc_check(const char *part_name, u32 addr, u32 size, u32 crc)
{
    ...
    while (pos < size)
    {
        ...
        {
            LOG_E("Qboot read firmware datas fail. part = %s, addr = %08X, length = %d", part_name,
                  pos, read_len);
            return false;
        }
...
static bool qbt_fw_pkg_read(const fal_partition_t part, u32 pos, u8 *buf, u32 read_len,
                            u8 *crypt_buf, int crypt_type)
{
    switch (crypt_type)
    {
    case QBOOT_ALGO_CRYPT_NONE:
...
static int qbt_dest_part_write(fal_partition_t part, u32 pos, u8 *decmprs_buf, u8 *cmprs_buf,
                               u32 *p_cmprs_len, int cmprs_type)
{
    int write_len = 0;
    int cmprs_len = 0;
    int decomp_len = 0;
    ...
    case QBOOT_ALGO_CMPRS_GZIP:
        ...
        while (1)
        {
            ...
            decomp_len = qbt_gzip_decompress(decmprs_buf + gzip_remain_len,
                                             QBOOT_BUF_SIZE - gzip_remain_len);
            if (decomp_len < 0)
            {
                write_len = -1;
                cmprs_len = 0;
            ...
            if (is_end && (cmprs_len < QBOOT_CMPRS_READ_SIZE)
                && (gzip_remain_len > 0)) // last package and remain > 0
            {
                memset(gzip_remain_buf + gzip_remain_len, 0xFF,
                       GZIP_REMAIN_BUF_SIZE - gzip_remain_len);
    ...
    case QBOOT_ALGO_CMPRS_QUICKLZ:
        while (1)
        {
            ...
            decomp_len
                = qbt_quicklz_decompress(decmprs_buf, cmprs_buf + QBOOT_QUICKLZ_BLOCK_HDR_SIZE);
            if (decomp_len <= 0)
            {
                write_len = -1;
                cmprs_len = 0;
    ...
    case QBOOT_ALGO_CMPRS_FASTLZ:
        while (1)
        {
            ...
            decomp_len = qbt_fastlz_decompress(decmprs_buf, QBOOT_BUF_SIZE,
                                               cmprs_buf + QBOOT_FASTLZ_BLOCK_HDR_SIZE, block_size);
            if (decomp_len <= 0)
            {
                write_len = -1;
                cmprs_len = 0;
...
static int qbt_app_crc_cal(u32 *p_crc32, u32 max_cal_len, u8 *decmprs_buf, u8 *cmprs_buf,
                           u32 *p_cmprs_len, int cmprs_type)
{
    int write_len = 0;
    int cmprs_len = 0;
    ...
    case QBOOT_ALGO_CMPRS_GZIP:
        ...
        while (1)
        {
            ...
            write_len += decomp_len;
            // last package and remain > 0
            if (is_end && (cmprs_len < QBOOT_CMPRS_READ_SIZE) && (gzip_remain_len > 0))
            {
...
static bool qbt_app_crc_check(const char *fw_part_name, fw_info_t *fw_info)
{
    ...
    while (app_cal_pos < fw_info->raw_size)
    {
        ...
        if (!qbt_fw_pkg_read(src_part, src_read_pos, cmprs_buf + cmprs_len, read_len, crypt_buf,
                             crypt_type))
        {
            qbt_fw_decompress_deinit(cmprs_type);
            LOG_E(
                "Qboot app crc check fail. read package error, part = %s, addr = %08X, length = %d",
                fw_part_name, src_read_pos, read_len);
            return false;
        }
...
static bool qbt_fw_release(const char *dst_part_name, const char *src_part_name, fw_info_t *fw_info)
{
    ...
    if ((fal_partition_erase(dst_part, 0, fw_info->raw_size) < 0)
        || (fal_partition_erase(dst_part, dst_part->len - sizeof(fw_info_t), sizeof(fw_info_t))
            < 0))
    {
        qbt_fw_decompress_deinit(cmprs_type);
    ...
    while (dst_write_pos < fw_info->raw_size)
    {
        ...
        {
            qbt_fw_decompress_deinit(cmprs_type);
            LOG_E("Qboot release firmware fail. read package error, part = %s, addr = %08X, length "
                  "= %d",
        ...
        write_len = qbt_dest_part_write(dst_part, dst_write_pos, crypt_buf, cmprs_buf, &cmprs_len,
                                        cmprs_type);
        if (write_len < 0)
        {
            qbt_fw_decompress_deinit(cmprs_type);
            LOG_E("Qboot release firmware fail. write destination error, part = %s, addr = %08X",
...
{
    factory_check_hook = hook;
}
#endif

#ifdef QBOOT_USING_FACTORY_KEY
static rt_tick_t   key_check_timeout_tick = 0;
static qbt_check_t qbt_factory_key_check(void)
{
    ...
    if (is_init == false)
    {
        ...
        {
#ifdef QBOOT_USING_STATUS_LED
            // 恢复出厂键按下，正在检测按键持续按下时间
            qled_set_blink(QBOOT_STATUS_LED_PIN, 50, 200);
#endif
            ret = QBT_CHECK_WAITING;
        }
    }
    else if (rt_pin_read(QBOOT_FACTORY_KEY_PIN) == QBOOT_FACTORY_KEY_LEVEL)
    {
        if (rt_tick_get() < key_check_timeout_tick)
        {
            ret = QBT_CHECK_WAITING;
        }
        else
...
static int qbt_factory_key_init(void)
{
...
#endif
    qbt_factory_check_set_hook(qbt_factory_key_check);
}
INIT_BOARD_EXPORT(qbt_factory_key_init);
#endif
...
static bool qbt_shell_init(const char *shell_dev_name)
{
    ...
    if (rt_device_open(dev, RT_DEVICE_OFLAG_RDWR | RT_DEVICE_FLAG_INT_RX | RT_DEVICE_FLAG_STREAM)
        != RT_EOK)
    {
...
static bool qbt_app_resume_from(const char *src_part_name)
{
    ...
    {
        LOG_E("Qboot resume fail from %s.", src_part_name);
        LOG_E("The firmware of %s partition is not application. fw_info.part_name(%s) != %s",
              src_part_name, fw_info.part_name, QBOOT_APP_PART_NAME);
        return false;
    }
...
#define QBOOT_REBOOT_DELAY_MS 5000
    ...
    for (;;)
    {
        if (factory_check_hook != NULL)
        {
            qbt_check_t check = factory_check_hook();
            switch (check)
            {
            case QBT_CHECK_TRUE:
            {
...
{
    rt_thread_t tid = rt_thread_create("Qboot", qbt_thread_entry, NULL, QBOOT_THREAD_STACK_SIZE,
                                       QBOOT_THREAD_PRIO, 20);
    if (tid == NULL)
    {
...
static bool qbt_fw_clone(const char *dst_part_name, const char *src_part_name, u32 fw_pkg_size)
{
    ...
    while (pos < fw_pkg_size)
    {
        ...
        {
            LOG_E("Qboot clone firmware fail. read error, part = %s, addr = %08X, length = %d",
                  src_part_name, pos, read_len);
            return false;
        }
        if (fal_partition_write(dst_part, pos, cmprs_buf, read_len) < 0)
        {
            LOG_E("Qboot clone firmware fail. write error, part = %s, addr = %08X, length = %d",
                  dst_part_name, pos, read_len);
            return false;
        }
        pos += read_len;
